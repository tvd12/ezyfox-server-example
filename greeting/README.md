...
1. greeting-app-api: contains app's request controllers, app's event handlers and components that are related to
   app
2. greeting-app-entry: contains AppEntryLoader class (you should not add any classes in this module)
3. greeting-app-entry/config/config.properties: app's configuration file
4. greeting-common: contains components that are used by both app and plugin
5. greeting-plugin: contains plugin's event handlers, plugin's request controllers and components that are related
   to plugin. You will need handle `USER_LOGIN` event here
6. greeting-plugin/config/config.properties: plugin's configuration file
7. greeting-startup: contains ApplicationStartup class to run on local (you should not add any classes in this
   module)
8. greeting-startup/src/main/resources/logback.xml: logback configuration file
...
You just move to `greeting-startup` module and run `ApplicationStartup`
...
4. Copy greeting-zone-settings.xml to EZYFOX_SERVER_HOME/settings/zones folder
5. Open file `EZYFOX_SERVER_HOME/settings/ezy-settings.xml` and add to `<zones>` tag:

```xml
<zone>
	<name>greeting</name>
	<config-file>greeting-zone-settings.xml</config-file>
	<active>true</active>
</zone>
```
...
1. Move to `greeting-startup` module
2. Run `mvn clean install -Denv.EZYFOX_SERVER_HOME=deploy -Pezyfox-deploy`
3. Run class `com.tvd12.greeting.tools.ExternalLibrariesExporter` in `greeting-startup/src/test/java`
...
2. Move to `greeting-startup/deploy` folder
...
1. greeting-app-api => `ezyfox-server/apps/common/greeting-app-api-1.0.0.jar`
2. greeting-app-entry => `ezyfox-server/apps/entries/greeting-app`
3. greeting-common => `ezyfox-server/common/ greeting-common-1.0.0.jar`
4. greeting-plugin => `ezyfox-server/plugins/greeting-plugin`
...
You just need use tool or `scp` to copy `greeting-startup/deploy` to your server
