package org.youngmonkeys.ezyfox_server_spring_example.tools;
        ...
        if (exportedLibraries.isEmpty()) {
            System.err.println(
                "you must run: "
                    ...
                    + "on ezyfox-sever-spring-example-startup module to export all libraries first"
            );
            return;
        }
    ...
    private static Path getDeployLibFolderPath() {
        ...
        if (!Files.exists(deployLibFolderPath)) {
            deployLibFolderPath = Paths.get(
                "ezyfox-sever-spring-example-startup/deploy/lib"
            );
        }
