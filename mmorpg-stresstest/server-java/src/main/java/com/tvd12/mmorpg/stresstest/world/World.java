...
public final class World extends AbstractHeartBeat {
	...
	private Map<String, EzyUser> __inspectors = new ConcurrentHashMap<>(); //Server.getInstance().getPlayerApi().gets();
//	private MessageApi __messageApi = Server.getInstance().getMessageApi();
	...
	@EzyAutoBind
	private EzyZoneContext zoneContext;
	@EzyAutoBind
	...
	public World(int cx, int cy) {
		...
		__currentCCU = 0; //Server.getInstance().getPlayerApi().count();
		__workersPool = new WorkersPool("world", 8, 16);
	...
	protected void _onUpdate(float delta) {
		...
		if (__sendingInterval >= SEND_PACKETS_RATE) {
			...
			__currentCCU = serverContext.getServer()
				.getSessionManager()
...
//					}
					...
					// update the vehicles
					List<EzySession> sessions = serverContext.getServer()
						.getSessionManager()
						.getLoggedInSessions();
					for (int i = 0; i < vehicles.size(); ++i) {
						...
						for(EzySession session : sessions) {
							appResponseFactory.newArrayResponse()
								.udpTransport()
								.command("p")
								.session(session)
								.params(
	...
	public void onUserGetNeighbours(EzyUser user) {
		// System.out.println("World._onMessage(): " + message.getContent().toString());
		try {
			__workersPool.execute(() -> {
				int entityId = (int)user.getId();
				if (entityId > __paramLoader.NUM_AGENTS - 1) {
					entityId = __paramLoader.NUM_AGENTS - 1;
				}

				List<Vehicle> neighbours = __getNeighboursOf(entityId);
				EzyUserManager userManager = zoneContext.getZone()
					.getUserManager();
				appResponseFactory.newArrayResponse()
					.command("r")
					.udpTransport()
					.params(_getFPS())
					.users(EzyLists.newArrayList(neighbours, it ->
							userManager.getUser(it.getIndex())
						)
					)
					.execute();
			}, user.getName());
